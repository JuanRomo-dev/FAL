...
int buscarIzq (vector<int> v, int ini, int final, int altura) {
    if (ini >= final) {             // Si ya hemos llegado al final devolvemos el valor y en resuelve caso decimos que si hemos llegado a este punto entonces no esta en la izquierda.
        return ini;
    }
    else {
        int m = (ini + final - 1) / 2;
